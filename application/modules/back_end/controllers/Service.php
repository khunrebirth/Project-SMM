...
class Service extends MX_Controller
{
	...
	public function service_store()
	{
		...
		$banner_img_en = '';
		$banner_img_th = '';
		$banner_img_moblie_en = '';
		$banner_img_moblie_th = '';
		...
		$input_content_top_img_title_alt = ['en' => $this->input->post('content_top_img_title_alt_en'), 'th' => $this->input->post('content_top_img_title_alt_th')];
		...
		$input_content_bottom_img_title_alt = ['en' => $this->input->post('content_bottom_img_title_alt_en'), 'th' => $this->input->post('content_bottom_img_title_alt_th')];
		...
		$input_banner_img_moblie = ['en' => $banner_img_moblie_en, 'th' => $banner_img_moblie_th];
		...
		$add_service = $this->Service_model->insert_service([
			...
			'banner_img_moblie' => serialize($input_banner_img_moblie),
			...
			'content_top_img_title_alt' => serialize($input_content_top_img_title_alt),
			...
			'content_bottom_img_title_alt' => serialize($input_content_bottom_img_title_alt),
	...
	public function service_update($lang, $service_id)
	{
		...
		$banner_img_moblie_en = unserialize($service->banner_img_moblie)['th'];
		$banner_img_moblie_th = unserialize($service->banner_img_moblie)['th'];
	...
	public function list_service_clients($lang, $service_id)
	{
		$this->data['lang'] = $this->lang;
		$this->data['title'] = 'Page: Services';
		$this->data['content'] = 'services/client_list';
		$this->data['service'] = $service = $this->Service_model->get_service_by_id($service_id);
		$this->data['clients'] =  $this->Service_client_model->get_service_client_by_service_id($service_id);

		$this->load->view('app', $this->data);
	}

	public function service_client_create($lang, $service_id)
	{
		$this->data['lang'] = $this->lang;
		$this->data['title'] = 'Page: Services - Client - Add';
		$this->data['content'] = 'services/client_create';
		$this->data['service'] = $service = $this->Service_model->get_service_by_id($service_id);

		$this->load->view('app', $this->data);
	}

	public function service_client_store($lang, $service_id)
	{
		// Handle Image
		$img_en = '';
		$img_th = '';
		...
		// Add Data
		$add_service_client = $this->Service_client_model->insert_service_client([
			'img' => serialize($input_img),
			'img_title_alt' => serialize($input_title),
			'service_id' => $service_id
		]);
		...
		// Set Session To View
		if ($add_service_client) {

			logger_store([
				'user_id' => $this->data['user']->id,
				'detail' => 'เพิ่ม Client (Services Page)',
				'event' => 'add',
				'ip' => $this->input->ip_address(),
			]);
		...
		redirect($this->lang . '/backoffice/page/services/list-service-clients/' . $service_id);
	}

	public function service_client_edit($lang, $service_id, $service_client_id)
	{
		$service = $this->Service_model->get_service_by_id($service_id);

		$this->data['lang'] = $this->lang;
		$this->data['title'] = 'Page: Services - Portfolio - Edit';
		$this->data['content'] = 'services/client_edit';
		$this->data['service'] = $service;
		$this->data['client'] =  $this->Service_client_model->get_service_client_by_id($service_client_id);

		$this->load->view('app', $this->data);
	}

	public function service_client_update($lang, $service_id, $service_client_id)
	{
		// Get Old data
		$service_client = $this->Service_client_model->get_service_client_by_id($service_client_id);

		// Handle Image
		$img_en = unserialize($service_client->img)['en'];
		$img_th = unserialize($service_client->img)['th'];
		...
		// Update Data
		$update_client = $this->Service_portfolio_model->update_service_portfolio_by_id($service_client_id, [
			'img' => serialize($input_img),
			'img_title_alt' => serialize($input_title),
			'updated_at' => date('Y-m-d H:i:s')
		]);
		...
		if ($update_client) {
			...
			logger_store([
				'user_id' => $this->data['user']->id,
				'detail' => 'แก้ไข Client (Services Page)',
				'event' => 'update',
				'ip' => $this->input->ip_address(),
			]);
	...
	public function service_client_destroy($lang, $service_client_id)
	{
		$status = 500;
		$response['success'] = 0;

		$service_client = $this->Service_client_model->delete_service_client_by_id($service_client_id);
		...
		if ($service_client != false) {
			...
			logger_store([
				'user_id' => $this->data['user']->id,
				'detail' => 'ลบ Client (Services Page)',
				'event' => 'delete',
				'ip' => $this->input->ip_address(),
			]);
		}
